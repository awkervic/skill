...
### 🧹 ai-cache-cleaner (工作区垃圾归档专家)

**指令关键词**：`/clean`, `清理缓存`

**技能定义**：
当用户触发此指令时，你必须执行以下归档动作，将工作区中的临时生成物移至 `ai归档/workspace_garbage/` 目录下集中保存，而非直接物理删除。核心业务文件则保留在原位。

1. **归档运行垃圾**：在当前工作空间根目录下，将所有生成的 `__pycache__` 文件夹**移动**到 `ai归档/workspace_garbage/` 下，按时间戳命名子目录（如 `ai归档/workspace_garbage/__pycache___20260608/`）。
2. **归档临时构建**：如果存在，将项目中的临时编译缓存（如 `.pytest_cache`、`.sass-cache`、`build/` 或 `dist/` 中的临时生成物，除非用户指定保留）移动到 `ai归档/workspace_garbage/` 下。
3. **归档中间脚本**：将工作区中所有以 `scratch_`、`temp_`、`analyze_`、`verify_` 等命名的临时 Python 脚本（不包含用户手动创建的核心业务文件）移动到 `ai归档/workspace_garbage/scratch_scripts/` 下。
4. **安全红线（严禁清除会话缓存）**：为保护大模型客户端的 Local Context Caching（上下文缓存）机制、防止重新加载完整历史导致 Token 暴涨，**严禁移动或删除** `C:\Users\Dell\.gemini\antigravity-cli\brain\` 下的任何 UUID 文件夹、`Task Logs`、`transcript.jsonl` 或会话草稿。
5. **安全边界**：禁止触碰任何用户手动创建的 `.py`, `.xlsx`, `.csv` 或其他各种数据文件。

**回复模板**：
归档完成后，列出已归档到 `ai归档/workspace_garbage/` 的工作区垃圾路径，并确认会话缓存已安全保留。
...
3. **归档冗余**：在确保上述信息已成功写入 `ai归档/LOGIC_BASE.md` 后，执行以下动作：
   - 将工作区中中间生成的 `scratch` 临时脚本**移动**到 `ai归档/workspace_garbage/` 下（仅限工作区，绝不触碰 `brain\<UUID>\` 目录）。
   - **严禁删除或移动** 当前会话的任何 `Task Logs`、日志文件或会话草稿，以保护上下文缓存，防范 Token 暴涨风险。
...
### 第三步：打扫战场与工作区归档 (3秒)
1. **执行工作区归档（而非物理删除）**：
   - 将工作区中所有中间生成的 `scratch` 临时 Python 脚本**移动**到 `ai归档/workspace_garbage/` 目录下（不触碰 `brain` 文件夹）。
   - 将当前工作区根目录下所有生成的 `__pycache__` 文件夹**移动**到 `ai归档/workspace_garbage/` 目录下，按时间戳命名。
   - 将项目中的临时编译缓存（如 `.pytest_cache`、`.sass-cache` 等）**移动**到 `ai归档/workspace_garbage/` 目录下。
   > **原则**：所有临时产物均以「移动（mv）」替代「删除（rm）」，存入 `ai归档/workspace_garbage/` 以保留审计追溯能力。
2. **严防死守缓存红线**：
   - **绝对禁止** 移动或删除 `C:\Users\Dell\.gemini\antigravity-cli\brain\<UUID>` 目录下的 `Task Logs`、临时草稿、日志或任何会话痕迹。必须完整保留会话缓存，以防因加载全新上下文而产生巨额 Token 消耗。
3. **恪守安全边界**：绝对不能触碰任何用户手动创建的 `.py`、`.xlsx`、`.csv` 或其他数据文件。

### 第四步：纯净重置响应模板
归档完毕后，根据以下模板给用户进行最终响应：
1. 简要列出本次提炼并保存的“核心思路条目”。
2. 列出已归档到 `ai归档/workspace_garbage/` 的工作区路径，并确认：*“核心灵魂已锁死在 ai归档/LOGIC_BASE.md，临时产物已安全归档保留，会话缓存完好无损。”*
3. 提示当前工作区已处于“极致整洁状态”，用户可以随时开启一个“全新的、零 Token 负担”的对话，只需引用 `ai归档/LOGIC_BASE.md` 和轨二的 `rules.md` 即可重回巅峰战力。如需找回归档文件，可前往 `ai归档/workspace_garbage/` 查看。
...
- **工作区归档**：
  - `ai归档/workspace_garbage/` (工作区临时产物的安全中转站，替代直接物理删除)
