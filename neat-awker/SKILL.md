...
### 🧹 ai-cache-cleaner (会话痕迹清理专家)

**指令关键词**：`/clean`, `清理缓存`

**技能定义**：
当用户触发此指令时，你必须以“极致洁癖”的模式执行以下物理清理动作，确保工作区只留下核心业务文件：

1. **确定会话路径**：识别当前会话在 `C:\Users\Dell\.gemini\antigravity-cli\brain\` 下对应的具体 UUID 文件夹。
2. **执行物理删除**：使用 PowerShell 命令递归删除该 UUID 文件夹下的所有 `Task Logs` 和临时草稿。
3. **清理运行垃圾**：在当前工作空间根目录下，强制删除所有生成的 `__pycache__` 文件夹。
4. **安全边界**：禁止触碰任何用户手动创建的 `.py`, `.xlsx`, `.csv` 或其他各种数据文件。

**回复模板**：
清理完成后，请列出已物理清除的路径，并告知用户当前工作区的“整洁状态”。
...
### 第三步：物理打扫战场与缓存清理 (3秒)
1. **定位会话路径**：在 `C:\Users\Dell\.gemini\antigravity-cli\brain\` 中识别出当前会话 UUID 对应的文件夹。
2. **物理深度删除**：
   - 物理删除该会话 UUID 文件夹下的所有 `Task Logs` 和临时草稿。
   - 物理删除工作区中所有中间生成的 `scratch` 临时 Python 脚本。
...
2. 列出已物理清除的路径，并确认：*“冗长对话数据已清理，核心灵魂已锁死在 ai归档/LOGIC_BASE.md。”*
