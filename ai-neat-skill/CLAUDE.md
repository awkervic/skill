...
1.  **Portability First**: Maintain `memory.md` and `Summarize.md` in the `<root>/ai归档/编年体记忆/` folder to ensure memory travels with the code.
2.  **Incremental Summary**: Append changes to `Summarize.md` under `<root>/ai归档/编年体记忆/` with Beijing Time `[YYYY-MM-DD HH:mm]`.
3.  **Sync to Root**: Extract snapshots and append them to `<root>/ai归档/编年体记忆/memory.md`.
