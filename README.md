...
### 1. 🧠 neat-awker (双轨制轻量化记忆与矩阵修剪技能)
* **目录**: [`neat-awker/`](./neat-awker/)
* **功能简介**: 解决长对话中上下文溢出与整理缓慢的问题。通过**轻量化双轨制机制**（轨一增量盲写追加，轨二洁癖修剪矩阵），极大提升运行速度并节省 Tokens。
* **核心资产**:
  * `SKILL.md`: 技能定义与激活协议。
  * `ai归档/编年体记忆/memory.md`: 只写不读的大事记存档。
  * `ai归档/编年体记忆/Summarize.md`: 增量变动日志（基于北京时间）。
  * `ai归档/核心规则/rules.md`: 实际指导开发的核心规章与矩阵修剪手册。
...
* **核心规范**:
  ...
  * **记忆同步**: 自动衔接并关联 `neat-awker` 维护的记忆系统。
